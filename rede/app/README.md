...
3. Nas regras de segurança, comece com o modo de teste (dá pra travar depois):
   ...
   > Firebase.
4. Ainda no menu lateral, vá em **Compilação > Storage** e clique em
   **Começar**. Escolha a mesma região do Realtime Database. Nas regras de
   segurança do Storage, comece (só pra testar) com:
   ```
   rules_version = '2';
   ...
   ```
   > Mesma lógica do modo de teste do Realtime Database: libera geral pra
   > testar rápido, sem precisar de login. Antes de divulgar o app
   > publicamente, vale restringir (ex.: limitar tamanho/tipo de arquivo).
5. Vá em **Configurações do projeto** (ícone de engrenagem) > aba **Geral** >
   role até "Seus apps" > clique no ícone **`</>`** (Web) > registre o app.
6. Copie o objeto `firebaseConfig` que aparece na tela.
7. Abra o arquivo `firebase-config.js` deste pacote e cole os valores no
   lugar dos `"SEU_..."`.
...
- **Anexar arquivo/imagem:** o clipe 📎 ao lado do campo de mensagem
  ...
  cartão baixável. Limite de 5MB por arquivo. O arquivo é enviado pro
  **Firebase Storage** (não mais como base64 dentro do Realtime
  Database) — só o link de download é que fica salvo junto da
  mensagem, então o banco de dados continua leve mesmo com bastante
  gente trocando arquivo.
- **Feedback de envio:** o botão "Enviar" mostra um spinner e fica
  desabilitado enquanto a mensagem (e os anexos, se houver) ainda
  estão subindo. Se a escrita falhar — conexão caiu, regra do Firebase
  bloqueou, etc. — aparece um aviso na tela e o texto/anexos voltam
  pro campo, pra não se perder o que foi digitado.
...
- O banco (e o Storage, usado pra anexos) em modo de teste são públicos:
  qualquer um com o link do site pode ler e escrever. Para algo mais sério,
  ajuste as regras do Realtime Database e do Storage, ou migre para o
